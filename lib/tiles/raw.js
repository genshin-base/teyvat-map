...
export async function forEachTileGroup(cfg, mapCode, tileFunc) {
	...
	if (mapCode === 'teyvat') {
		tileRe = /UI_MapBack_(-?\d+)_(-?\d+).*\.png/
		tilePrefix = 'UI_MapBack_'
	} else if (mapCode === 'enkanomiya') {
		tileRe = /UI_MapBack_AbyssalPalace_(-?\d+)_(-?\d+).*\.png/
		tilePrefix = 'UI_MapBack_AbyssalPalace_'
	} else if (mapCode === 'chasm') {
		tileRe = /UI_MapBack_TheChasm_(-?\d+)_(-?\d+).*\.png/
	...
	for (let j = tilesRect.top; j >= tilesRect.bottom; j--) {
		for (let i = tilesRect.left; i >= tilesRect.right; i--) {
			const tiles = /**@type {{fpath:string, bytes:Buffer}[]}*/ ([])

			for (const fpath of allFPaths) {
				const prefix = `${tilePrefix}${j}_${i}`
				const fname = basename(fpath)
				if (fname.startsWith(prefix) && !/^\d/.test(fname.slice(prefix.length))) {
					const bytes = await fs.readFile(fpath)
					if (!tiles.some(x => x.bytes.equals(bytes))) {
						tiles.push({ fpath, bytes })
					}
				}
			}

			const tileFPaths = tiles.map(x => x.fpath).sort()
